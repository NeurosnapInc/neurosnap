...
def conformer_find_LCS(mol):
  ...
  """
  print("[*] Finding largest common substructure (LCS) for clustering")
  ...
  print("[+] Core substructure detected. Aligning using core.")
  AllChem.AlignMolConformers(mol, mol.GetSubstructMatch(core))
  return mol


def conformer_minimize(mol, method="MMFF94", e_delta=5):
  """
  -------------------------------------------------------
  ...
  """
  print(f"[*] Minimizing energy of all conformers using {method}")
  ...
  print(f"[*] Filtered {num_filtered:,} conformers within 0 to {energy_threshold:.2f} kcal/mol.")
  print(f"[+] min: {min(energies.values()):.2f}, max: {max(energies.values()):.2f}, avg: {sum(energies.values())/len(energies.values()):.2f}")
  return mol_filtered, energies_filtered


def conformers_generate(input_mol, output_name="unique_conformers", write_multi=False, num_confs=1000, min_method="MMFF94"):
  """
  -------------------------------------------------------
  ...
  print(f"[*] Generated {my_mol.GetNumConformers():,} 3D conformers.")

  ### Minimize energy of each conformer
  if min_method:
    my_mol, energies = conformer_minimize(my_mol, method=min_method)
  ...
  print(f"Dynamic RMSD threshold: {threshold:.2f} Å")
  ...
  for i, cluster in enumerate(clusters, start=1):
    ...
    print(f"Cluster ID: {i}, Best Conformer: {best_cid} ({best_energy:.2f}), Conformers {cluster}")
  ...
  print(f"[*] Selected {len(df)} unique conformers.")
  print(df)
